...
<div class="flx jcsa acfs acfs" style="width:100%;">
	...
	<div style="overflow:auto;width:100%;max-width:768px;margin:auto;">
	{foreach from=$tweets item=tweet}
	{if ((isset($twitterLogin) && !empty($twitterLogin)) || !$tweet['sensitive'])}
		<div style="min-width:75%;margin:10px;">
			<div style="width:100%;overflow:hidden;white-space:nowrap;">
				<a href="/timeline/user.php?domain=twitter&target_id={$tweet['account']['id']}&hs=true&thumb=true" target="_blank"><img src="{$tweet['account']['profileImage']}">{$tweet['account']['displayName']}</a> {date4timeline($tweet['date'])}
			</div>
			<hr>
			{$tweet['text']}
			<a class="popup-modal" href="#inline-wrap-{$tweet['id']}" target="_blank">
			<div style="max-width:80%;margin:auto;max-height:85vh;overflow:auto;">
				{if $tweet['isVideo']}
			...
			<div id="inline-wrap-{$tweet['id']}" style="max-width:80vw;overflow:auto;margin:auto;background-color:black;" class="mfp-hide">
				{if $tweet['isVideo']}
					{foreach from=$tweet['media'] item=media}
					<video style="max-width:80vw;margin:auto;display:block;" src="{$media['url']}" poster="{$media['thumb']}" preload="metadata" controls></video><br>
					{/foreach}
				{else}
					{foreach from=$tweet['media'] item=media}
					<img style="max-width:80vw;margin:auto;display:block;" src="{$media['url']}"><br>
					{/foreach}
				{/if}
			</div>

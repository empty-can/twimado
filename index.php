...
if(!empty($from_date) && !empty($to_date)) {
	$from_time = date('Y-m-d 00:00:00', strtotime($from_date));
	$to_time = date('Y-m-d 23:59:59', strtotime($to_date));
	$tweets = getFavRanking($from_time, $to_time);
	$smarty->assign("title", date('Y/m/d', strtotime($from_date))."～".date('Y/m/d', strtotime($to_date))."の画像");

} else if($t=='n') {
	$tweets = getNewTweet(100);
    ...
    if($t=='t') {
		...
		$from_time = date('Y-01-01 00:00:00');
		$to_time = date('Y-m-d H:i:s',strtotime('-1 day'));
		$smarty->assign("title", "今年の画像");

	} else {
		$from_time = $t.'-01-01 00:00:00';
		$to_time = $t.'-12-31 23:59:59';
...
$smarty->assign("t", $t);
$smarty->assign("from", $from_date);
$smarty->assign("to", $to_date);
